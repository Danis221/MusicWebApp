...
public class EventDaoImpl implements EventDao {
    ...
    //language=SQL
    private static final String SQL_SELECT_ALL = "select * from event order by concertDate desc;";
    ...
    public void save(Event event) {
        try {
            ...
            preparedStatement.setDate(2, event.getConcertDate());
    ...
    public void update(Event event) {
        try {
            ...
            statement.setDate(2, event.getConcertDate());
    ...
    public List<Event> getAllBySettings(Date startDate, Date endDate, Integer price) {
        try {
            //language=SQL
            StringBuilder sql = new StringBuilder("SELECT * from event ");
            ...
            } else if (startDate != null && endDate != null && price == null) {
                sql.append("where concertDate between ? and ? order by concertDate;");
                statement = connection.prepareStatement(String.valueOf(sql));
                statement.setDate(1, startDate);
                statement.setDate(2, endDate);
            } else if (startDate != null && endDate != null && price != null) {
                sql.append("where concertDate between ? and ? and price <= ? order by concertDate;");
                statement = connection.prepareStatement(String.valueOf(sql));
                statement.setDate(1, startDate);
                statement.setDate(2, endDate);
